...
class Animator():
	...
	def plot_drones(self, drones, learning_mode):
		plt.cla()
		for d in drones:
			self.plot_drone(d, learning_mode)
	...
	def plot_drone(self, d, learning_mode):
		...
		if learning_mode == 'store_points':
			self.traj_color = 'b'
		else:
			self.traj_color = 'r'

		self.ax.plot(d.x_data, d.y_data, d.z_data, self.traj_color + ':')
